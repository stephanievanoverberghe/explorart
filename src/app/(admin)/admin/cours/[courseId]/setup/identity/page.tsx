...
type Level = 'Débutant' | 'Intermédiaire' | 'Avancé' | 'Tous niveaux';
...
export default function SetupIdentityPage() {
    ...
    // TODO: remplacer par fetch du cours (server → props / hook / swr). Ici: stub UI.
    const [title, setTitle] = useState('');
    const [pillar, setPillar] = useState<Pillar>('dessin-peinture');
    ...
    async function saveDraft() {
        ...
        setSubmitting(true);
        try {
            // TODO: updateCourseIdentity(courseId, { title, pillar, level, access, pinned })
            const now = new Date();
                ...
                <CardBody>
                    <div className="space-y-6">
                        ...
                        <div className="grid gap-4 sm:grid-cols-2">
                            <Field label="Pilier *">
                                <PrettySelect value={pillar} onChange={(v) => setPillar(v as Pillar)} icon={<Tag className="h-4 w-4 text-main/45" />}>
                            ...
                            <Field label="Niveau *">
                                <PrettySelect value={level} onChange={(v) => setLevel(v as Level)} icon={<GraduationCap className="h-4 w-4 text-main/45" />}>
