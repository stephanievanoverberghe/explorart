...
interface SetupAccessClientProps {
    courseId: string;
    initialAccess: CourseAccessData;
}

export default function SetupAccessClient({ courseId, initialAccess }: SetupAccessClientProps) {
    const router = useRouter();

    const [access, setAccess] = useState<CourseAccess>(initialAccess.access);
    const [hasFreePreview, setHasFreePreview] = useState(initialAccess.hasFreePreview);
    const [requiresAccount, setRequiresAccount] = useState(initialAccess.requiresAccount);
