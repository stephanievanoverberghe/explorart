...
type Module = { id: string; title: string; goal: string; minutes: number };
function uid() {
...
export default function SetupStructurePage({ params }: { params: { courseId: string } }) {
    const router = useRouter();
    const courseId = params.courseId;

    const [introMinutes, setIntroMinutes] = useState(5);
    const [conclusionMinutes, setConclusionMinutes] = useState(5);
    ...
    async function saveDraft() {
        ...
        setSubmitting(true);
        try {
            // TODO: updateCourseStructure(courseId, { introMinutes, modules, conclusionMinutes })
            const now = new Date();
